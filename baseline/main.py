# -*- coding: utf-8 -*-
import argparse # 명령줄 인터페이스로 파라미터를 설정하고, 전달하기 위한 라이브러리
import datetime # 날짜 및 시간 관련 기능을 제공하는 파이썬 내장 모듈
import inspect # 파이썬 객체의 정보를 검색하기 위한 기능을 제공하는 모듈
import os # 운영체제와 상호작용을 위한 기능을 제공하는 모듈
import time # 시간 관련 기능을 제공하는 파이썬 내장 모듈
from pprint import pprint # 데이터를 보기좋게 출력하는 모듈

import pandas as pd # 데이터 조작 및 분석을 위한 라이브러리
import numpy as np # 수치계산을 위한 라이브러리

import torch # 딥러닝 프레임워크
from torch.utils.data import DataLoader # torch.utils.data >> 데이터의 효율적인 로드 및 사용을 위한 모듈
from torch import nn 

from data_utils.Desed import DESED # (이 부분 부터는 사용자 정의 모듈 및 클래스에 해당)
...
def adjust_learning_rate(optimizer, rampup_value, rampdown_value=1): 
    # 학습률 조절 함수(optimizer:파이토치의 optimizer객체 / rampup_val, rampdown_val : 둘다 0~1사이의 값으로, learning rate가 선형적으로 증가, 감소하는 정도를 결정)
